...
"""One-read-per-state K11/K12 model phase for the K12 forward scan."""
...
_WORKER_EXISTING_INDEX: int | None = None
...
def _fit_one(
    *,
    ...
    if new_envelope_index is None:
        support = tuple(range(K11))
    else:
        support = tuple(range(K12))
    if new_envelope_index is None:
        phi_a = k11_a
        phi_a_b = k11_a_b
        phi_c = k11_c
        ...
        phi_a = np.column_stack((k11_a, env_a[:, int(new_envelope_index)]))
        phi_a_b = np.column_stack((k11_a_b, env_a_b[:, int(new_envelope_index)]))
        phi_c = np.column_stack((k11_c, env_c[:, int(new_envelope_index)]))
        phi_c_b = np.column_stack((k11_c_b, env_c_b[:, int(new_envelope_index)]))
...
def worker_init(
    ...
    existing_envelope_index: int,
) -> None:
    global _WORKER_CANDIDATES, _WORKER_TERMS, _WORKER_EXISTING_INDEX, _WORKER_STATE_TABLE
    _WORKER_CANDIDATES = tuple(
        (int(candidate_id), None if new_index is None else int(new_index))
        for candidate_id, new_index in candidate_specs
    )
    _WORKER_TERMS = tuple(build_envelope_dictionary())
    _WORKER_EXISTING_INDEX = int(existing_envelope_index)
    if len(_WORKER_TERMS) != 75:
        raise RuntimeError("Envelope75 dictionary count changed")
    if len(_WORKER_CANDIDATES) != 65:
        raise RuntimeError("K12 forward scan requires one K11 baseline and 64 K12 candidates")
    if _WORKER_CANDIDATES[0][1] is not None:
        raise RuntimeError("candidate 0 must be the K11 baseline")
    if _WORKER_TERMS[_WORKER_EXISTING_INDEX].basis_id != BEST_K11_BASIS_ID:
        raise RuntimeError("frozen K11 basis index does not point to ENV_p04_m2_q0")
    if any(index is not None and not 0 <= index < len(_WORKER_TERMS) for _, index in _WORKER_CANDIDATES):
        raise RuntimeError("K12 candidate Envelope75 index is invalid")
    _WORKER_STATE_TABLE = {int(row["state_id"]): row for row in build_state_table()}


def state_worker(state_id: int) -> dict[str, Any]:
    if _WORKER_CANDIDATES is None or _WORKER_TERMS is None or _WORKER_EXISTING_INDEX is None or _WORKER_STATE_TABLE is None:
        raise RuntimeError("K12 worker is not initialized")
    ...
    k11_a = np.column_stack((mp_a, env_a[:, _WORKER_EXISTING_INDEX]))
    k11_a_b = np.column_stack((mp_a_b, env_a_b[:, _WORKER_EXISTING_INDEX]))
    k11_c = np.column_stack((mp_c, env_c[:, _WORKER_EXISTING_INDEX]))
    k11_c_b = np.column_stack((mp_c_b, env_c_b[:, _WORKER_EXISTING_INDEX]))
    actual_shapes = {
        "k11_a": k11_a.shape,
        "k11_a_b": k11_a_b.shape,
        "k11_c": k11_c.shape,
        "k11_c_b": k11_c_b.shape,
    }
    expected_shapes = {
        "k11_a": (12_286, K11),
        "k11_a_b": (FINGERPRINT_LENGTH, K11),
        "k11_c": (7_371, K11),
        "k11_c_b": (FINGERPRINT_LENGTH, K11),
    }
    if actual_shapes != expected_shapes:
        raise RuntimeError(f"state {normalized_id} K11 shape changed: {actual_shapes}")
    ...
    candidate_results = [
        ...
        for candidate_id, new_index in _WORKER_CANDIDATES
    ]
    if any(not result["finite"] for result in candidate_results):
        raise RuntimeError(f"state {normalized_id} has a non-finite K11/K12 fit")
