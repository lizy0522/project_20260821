...
"""One-read-per-state K11/K10 backward model phase."""
...
_WORKER_ADDED_INDEX: int | None = None
...
def _fit_one(
    *,
    ...
    if remove_index is None:
        ...
        support = tuple(index for index in range(K11) if index != int(remove_index))
    ...
    padded_a = np.zeros(K11, dtype=np.complex128)
    padded_c = np.zeros(K11, dtype=np.complex128)
...
def worker_init(
    ...
    added_envelope_index: int,
) -> None:
    global _WORKER_CANDIDATES, _WORKER_TERMS, _WORKER_ADDED_INDEX, _WORKER_STATE_TABLE
    ...
    _WORKER_ADDED_INDEX = int(added_envelope_index)
    if len(_WORKER_TERMS) != 75:
        raise RuntimeError("Envelope75 dictionary count changed")
    if len(_WORKER_CANDIDATES) != 12:
        raise RuntimeError("backward scan requires one K11 baseline and 11 K10 deletions")
    if _WORKER_CANDIDATES[0][1] is not None:
        raise RuntimeError("candidate 0 must be the K11 baseline")
    if not 0 <= _WORKER_ADDED_INDEX < len(_WORKER_TERMS):
        raise RuntimeError("K11 added Envelope75 index is invalid")
    if _WORKER_TERMS[_WORKER_ADDED_INDEX].basis_id != BEST_K11_BASIS_ID:
        raise RuntimeError("K11 added basis ID does not match ENV_p04_m2_q0")
    _WORKER_STATE_TABLE = {int(row["state_id"]): row for row in build_state_table()}


def state_worker(state_id: int) -> dict[str, Any]:
    if _WORKER_CANDIDATES is None or _WORKER_TERMS is None or _WORKER_ADDED_INDEX is None or _WORKER_STATE_TABLE is None:
        raise RuntimeError("K11 backward worker is not initialized")
    ...
    aend_pair = get_ilc_pair(data, int(input_history.shape[1] - 1))
    ...
    full_a = np.column_stack((mp_a, env_a[:, _WORKER_ADDED_INDEX]))
    full_a_b = np.column_stack((mp_a_b, env_a_b[:, _WORKER_ADDED_INDEX]))
    full_c = np.column_stack((mp_c, env_c[:, _WORKER_ADDED_INDEX]))
    full_c_b = np.column_stack((mp_c_b, env_c_b[:, _WORKER_ADDED_INDEX]))
    expected = {
        "full_a": (12_286, K11),
        "full_a_b": (FINGERPRINT_LENGTH, K11),
        "full_c": (7_371, K11),
        "full_c_b": (FINGERPRINT_LENGTH, K11),
    }
    actual = {
        "full_a": full_a.shape,
        "full_a_b": full_a_b.shape,
        "full_c": full_c.shape,
        "full_c_b": full_c_b.shape,
    }
    if actual != expected:
        raise RuntimeError(f"state {normalized_id} K11 Phi shape changed: {actual}")
    ...
    if any(not result["finite"] for result in candidate_results):
        raise RuntimeError(f"state {normalized_id} has a non-finite K11/K10 fit")
