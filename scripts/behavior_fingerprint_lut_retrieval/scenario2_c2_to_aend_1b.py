...
from behavior_model.config import MAX_DELAY, MP_CONFIG, NUM_COEFFICIENTS  # noqa: E402
from behavior_model.evaluation import calculate_nmse  # noqa: E402
from behavior_model.ridge import fit_coefficients_ridge  # noqa: E402
from data_manager import build_state_table, get_state_info, load_by_id  # noqa: E402
...
RESULT_ROOT = PROJECT_ROOT / "results" / TASK_NAME / "scenario_2_C2_to_Aend_1B"
ALL_ILC_ROOT = (
...
LOW_BANDWIDTH_ROOT = (
    PROJECT_ROOT / "results" / "low_bandwidth_observation" / "sample_rate_operator_bank"
)
...
SPEC_0P5B = ObservationExperimentSpec(
    ...
    result_root=PROJECT_ROOT / "results" / TASK_NAME / "scenario_2_C2_to_Aend_0p5B",
)
...
def protection_snapshot() -> dict[str, Any]:
    protected = {
        ...
        "scenario_2_C2_to_Aend_equal_ABC": (
            PROJECT_ROOT / "results" / TASK_NAME / "scenario_2_C2_to_Aend_equal_ABC"
        ),
...
def _compute_cnmse_distance_matrix(query: np.ndarray, candidate: np.ndarray) -> np.ndarray:
    """Batch form of the frozen _core.metrics.cnmse formula for any length."""
