"""Run fixed K18 Aend/C2 Full-425 retrieval under the shared 0.5B operator."""
...
from behavior_fingerprint_retrieval.scenario_2.scenario_2_k18_aend_c2_full_lut_retrieval_0p5b.plot_scenario_2_k18_aend_c2_full_lut_retrieval_0p5b import (  # noqa: E402
    write_figures,
)
...
def _fit_side(segment: Any, operator: LowBandwidthObservationOperator, terms: tuple[EnvelopeBasis, ...], support: tuple[int, ...], expected_length: int) -> tuple[dict[str, object], np.ndarray, np.ndarray]:
    ...
    if phi_obs.shape != (expected_length, MODEL_K) or y_obs.shape != (expected_length,):
        raise RuntimeError(f"0.5B fit shape mismatch: {phi_obs.shape}, {y_obs.shape}")
...
def state_worker(state_id: int) -> dict[str, object]:
    if any(item is None for item in (_WORKER_TERMS, _WORKER_SUPPORT, _WORKER_COMMON_PHI, _WORKER_STATE_TABLE, _WORKER_OPERATOR, _WORKER_OBS_LENGTHS)):
        raise RuntimeError("0.5B worker is not initialized")
    ...
    if phi_a_b_obs.shape != (_WORKER_OBS_LENGTHS["B"], MODEL_K) or phi_c_b_obs.shape != (_WORKER_OBS_LENGTHS["B"], MODEL_K):
        raise RuntimeError(f"state {state_id} 0.5B B length mismatch")
...
def _top1(distance: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    if distance.shape != (STATE_COUNT, STATE_COUNT) or np.isnan(distance).any() or np.isposinf(distance).any():
        raise RuntimeError("0.5B distance matrix invalid")
...
def _write_checkpoint(phase: str, **payload: object) -> None:
    _write_json(RESULT_ROOT / "09_checkpoint.json", {"task_name": TASK_NAME, "phase": phase, "state_count": STATE_COUNT, "K": MODEL_K, "lambda": RIDGE_LAMBDA, "bandwidth": "0.5B", "Fs_obs_Hz": FS_OBS_HZ, "observation_index_k": OBSERVATION_INDEX_K, "support_hash": MODEL_SUPPORT_HASH, **payload})
...
def run(*, allow_existing: bool = False) -> dict[str, object]:
    ...
    WORK_LOG.parent.mkdir(parents=True, exist_ok=True)
    _append_log("START fixed K18 0.5B Aend/C2 Full-425 retrieval")
    ...
    if operator.spec.normalized_bandwidth_n != OBSERVATION_N or operator.spec.fs_out_hz != FS_OBS_HZ or operator.spec.up != 1 or operator.spec.down != 10:
        raise RuntimeError("0.5B observation operator contract failed")
    ...
    if lut.shape != (STATE_COUNT, obs_lengths["B"]) or query.shape != lut.shape or not np.all(np.isfinite(lut)) or not np.all(np.isfinite(query)):
        raise RuntimeError("0.5B fingerprint shape/finite contract failed")
    ...
    np.save(RESULT_ROOT / "04_fingerprint_distance_matrix_0p5b.npy", distance)
    np.savez_compressed(RESULT_ROOT / "05_lut_fingerprints_0p5b.npz", state_ids=ids, fingerprints=lut, common_B_input=common_b, observation_index_k=np.array(OBSERVATION_INDEX_K, dtype=np.int64))
    np.savez_compressed(RESULT_ROOT / "06_query_fingerprints_0p5b.npz", state_ids=ids, fingerprints=query, common_B_input=common_b, observation_index_k=np.array(OBSERVATION_INDEX_K, dtype=np.int64))
    np.savez_compressed(RESULT_ROOT / "10_model_coefficients_0p5b.npz", state_ids=ids, theta_Aend=theta_a, theta_C2=theta_c)
    ...
    raw_after = _raw_gate()
    summary = {"status": "NUMERICAL_COMPLETE", "task_name": TASK_NAME, "module": "behavior_fingerprint_retrieval", "bandwidth": "0.5B", "B_MHz": 20, "Fs_original_MHz": 100, "Fs_observation_MHz": 10, "resampling_ratio": "1/10", "operator_name": common_meta["operator"]["resampler"], "operator_metadata": common_meta["operator"], "state_count": STATE_COUNT, "allow_self": True, "model_candidate": MODEL_CANDIDATE, "K": MODEL_K, "lambda": RIDGE_LAMBDA, "dmax": DMAX, "p_max": MODEL_P_MAX, "support_hash": MODEL_SUPPORT_HASH, "Aend_definition": "last valid ILC column", "C2_definition": "literal second ILC column", "Common_B_sha256": COMMON_B_SHA256, "observed_segment_lengths": obs_lengths, "fingerprint_length": int(obs_lengths["B"]), "distance_matrix_shape": list(distance.shape), **counts, "self_rate": counts["N_self"] / STATE_COUNT, "valid_rate": counts["N_valid"] / STATE_COUNT, "fail_rate": counts["N_fail"] / STATE_COUNT, "fail_states": np.flatnonzero(fail).astype(int).tolist(), "mean_abs_state_delta": float(np.mean(delta)), "median_abs_state_delta": float(np.median(delta)), "max_abs_state_delta": int(np.max(delta)), "finite_real_B_CNMSE": finite_real, "shareability_threshold_dB": REAL_B_THRESHOLD_DB, "shareability_operator": "<", "raw_manifest_before": raw_before, "raw_manifest_after": raw_after, "raw_unchanged": raw_before == raw_after, "model_metric_medians": {column: float(frame[column].median()) for column in ("Y_Aend_train_NMSE_dB", "Y_Aend_B_NMSE_dB", "Y_C2_train_NMSE_dB", "Y_C2_B_NMSE_dB")}, "figure_metadata": figure_meta, "model_search_started": False, "nested_cv_started": False, "self_first_runner_started": False, "construction_order": "5B K18 basis -> shared 0.5B observation operator", "final_validation_domain": "5B Real-B yout_withoutdpd_ori B segment", "outputs": {"csv": str(RESULT_ROOT / "01_state_results.csv"), "summary": str(RESULT_ROOT / "02_retrieval_summary.json"), "model_definition": str(RESULT_ROOT / "03_model_definition.json"), "distance": str(RESULT_ROOT / "04_fingerprint_distance_matrix_0p5b.npy"), "xlsx": str(RESULT_ROOT / "scenario_2_k18_aend_c2_full_lut_retrieval_0p5b.xlsx"), "figure_01": str(RESULT_ROOT / "figure_01_full_metrics_vs_state_0p5b.png"), "figure_02": str(RESULT_ROOT / "figure_02_retrieved_real_B_CNMSE_vs_state_0p5b.png")}, **baseline}
    _write_json(RESULT_ROOT / "02_retrieval_summary.json", summary)
    summary_lines = [f"Task: {TASK_NAME}", "Status: NUMERICAL_COMPLETE; final SUCCESS awaits artifact-tool Excel and independent validator.", f"Observation: 0.5B = 10 MHz; original Fs=100 MHz; ratio=1/10; operator={common_meta['operator']['resampler']}", f"Observed lengths: {obs_lengths}; fingerprint length={obs_lengths['B']}", f"Model: {MODEL_CANDIDATE}; K={MODEL_K}; lambda={RIDGE_LAMBDA:.12g}; support_hash={MODEL_SUPPORT_HASH}", "LUT=Aend; Query=C2; allow_self=true; final Real-B validation uses 5B data.", "", *[f"{key}={value}" for key, value in counts.items()], f"self_rate={summary['self_rate']}", f"valid_rate={summary['valid_rate']}", f"fail_states={summary['fail_states']}", f"mean_abs(State_Q-State_R)={summary['mean_abs_state_delta']}", f"median_abs(State_Q-State_R)={summary['median_abs_state_delta']}", f"max_abs(State_Q-State_R)={summary['max_abs_state_delta']}", f"finite Real-B={json.dumps(finite_real, ensure_ascii=False, sort_keys=True)}", f"model_metric_medians={json.dumps(summary['model_metric_medians'], ensure_ascii=False, sort_keys=True)}", "Outputs: strict 10-column CSV source, artifact-tool Excel, two figures with all R->Q labels."]
