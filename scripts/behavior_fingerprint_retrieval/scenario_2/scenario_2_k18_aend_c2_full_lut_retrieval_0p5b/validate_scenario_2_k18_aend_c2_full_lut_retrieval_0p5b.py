"""Independent validator for the fixed K18 0.5B retrieval task."""
...
from behavior_fingerprint_retrieval.scenario_2.scenario_2_k18_aend_c2_full_lut_retrieval_0p5b.run_scenario_2_k18_aend_c2_full_lut_retrieval_0p5b import (  # noqa: E402
    COMMON_B_SHA256,
    EXPECTED_RAW_BYTES,
...
def _task_import_violations() -> list[str]:
    ...
    package = "behavior_fingerprint_retrieval.scenario_2.scenario_2_k18_aend_c2_full_lut_retrieval_0p5b"
...
def validate() -> dict[str, object]:
    failures: list[str] = []
    required = ["01_state_results.csv", "02_retrieval_summary.json", "03_model_definition.json", "04_fingerprint_distance_matrix_0p5b.npy", "05_lut_fingerprints_0p5b.npz", "06_query_fingerprints_0p5b.npz", "07_final_result_summary.txt", "09_checkpoint.json", "10_model_coefficients_0p5b.npz", "11_state_retrieval_diagnostics.csv", "figure_01_full_metrics_vs_state_0p5b.png", "figure_02_retrieved_real_B_CNMSE_vs_state_0p5b.png", "scenario_2_k18_aend_c2_full_lut_retrieval_0p5b.xlsx"]
    ...
    distance = np.asarray(np.load(RESULT_ROOT / "04_fingerprint_distance_matrix_0p5b.npy", allow_pickle=False), dtype=np.float64)
    with np.load(RESULT_ROOT / "05_lut_fingerprints_0p5b.npz", allow_pickle=False) as lut_data, np.load(RESULT_ROOT / "06_query_fingerprints_0p5b.npz", allow_pickle=False) as query_data:
    ...
    checks["observation"] = _check(summary.get("bandwidth") == "0.5B" and summary.get("Fs_observation_MHz") == 10 and summary.get("resampling_ratio") == "1/10", failures, "observation")
    checks["operator"] = _check(summary.get("operator_name") == "scipy.signal.resample_poly" and summary.get("operator_metadata", {}).get("up") == 1 and summary.get("operator_metadata", {}).get("down") == 10, failures, "operator")
    ...
    if not failures:
        ...
        with (PROJECT_ROOT / "work_logs" / "core" / "codex_handoff" / "codex_handoff.txt").open("a", encoding="utf-8") as handle:
            handle.write(f"\n{time.strftime('%Y-%m-%d')} | 新任务：{TASK_NAME}\n完成固定 K18 0.5B Aend/C2 Full-425 检索；artifact-tool 严格 10 列 Excel、6/1 曲线、全部 R→Q 标注、5B Real-B 验证和 raw manifest 均通过；未启动任何模型搜索。\n")
    return report
